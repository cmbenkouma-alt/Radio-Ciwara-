(() => {
  ...
  function getHeroColumns() {
    ...
    if (!grid) return null;
    const cols = [...grid.querySelectorAll('.hero-video-col, .hero-news-col, .ciwara-hero-video-col, .ciwara-caster-card')];
    if (cols.length < 2) return null;
    const caster = document.getElementById('74o6tf') || grid.querySelector('.cstrEmbed');
    const casterCol = caster?.closest('.hero-video-col, .hero-news-col, .ciwara-hero-video-col, .ciwara-caster-card');
    const sliderCol = cols.find((col) => col !== casterCol && !col.contains(caster)) || cols[0];
  ...
  async function getEmissionItems() {
    ...
    const add = (a) => {
      ...
      const text = clean(a.innerText || img?.alt || img?.title, 120);
    ...
    try {
      ...
      if (response.ok) {
        ...
        doc.querySelectorAll('article a, .emission-card a, .emission-item a, .program-card a, .card a, a').forEach(add);
      }
    } catch (_) {}
    if (candidates.length < 3) {
      document.querySelectorAll('.emission-card a, .emission-item a, .program-card a, article a, .card a').forEach(add);
    }
  ...
  function makeEmissionStage(items) {
    ...
    const slides = items.length ? items : [{ src: 'assets/ciwara-logo.jpg', href: 'emissions.html', title: 'Découvrez les émissions de Radio Ciwara' }];

    stage.innerHTML = `
      <div class="future-kicker"><strong>CIWARA • ÉMISSIONS</strong><span class="future-live-dot">PROGRAMME EN CONTINU</span></div>
      <div class="future-emission-slider">
        ${slides.map((item, i) => `
          <article class="future-emission-slide${i === 0 ? ' is-active' : ''}" data-index="${i}">
            <div class="future-emission-copy"><span class="eyebrow">Émission Radio Ciwara</span><h2>${esc(item.title)}</h2><p>Retrouvez vos rendez-vous, vos animateurs et toute l'énergie de Radio Ciwara 105.5 FM.</p></div>
            <a class="future-emission-art" href="${esc(item.href)}" aria-label="Ouvrir ${esc(item.title)}"><img src="${esc(item.src)}" alt="${esc(item.title)}" loading="eager"></a>
          </article>`).join('')}
      </div>
  ...
  async function buildEmissionHero() {
    ...
    if (parts.casterCol === sliderCol || sliderCol.contains(document.getElementById('74o6tf'))) return;
    sliderCol.innerHTML = '';
  ...
  function renderNewsStage(items) {
    ...
    stage.innerHTML = `
      <div class="ciwara-news-stage-head"><div><span class="ciwara-rss-pill">● CIWARA INFOS • FLUX RSS</span><h2>L'actualité <span>en mouvement</span></h2><p>Les dernières informations du Mali et les flux éditoriaux de Ciwara Infos.</p></div></div>
      <div class="ciwara-news-track">
        ${items.slice(0, 12).map((n) => `<article class="ciwara-news-card"><a href="${esc(n.url)}" target="_blank" rel="noopener noreferrer">${n.image ? `<img src="${esc(n.image)}" alt="" loading="lazy">` : '<div style="height:190px;background:linear-gradient(135deg,#1769ff,#ff3f91)"></div>'}<div class="ciwara-news-card-body"><small>${esc(n.source)}</small><h3>${esc(n.title)}</h3>${n.description ? `<p>${esc(n.description)}</p>` : ''}</div></a></article>`).join('')}
      </div>`;
  ...
  async function buildNewsStage() {
    ...
    const items = [...document.querySelectorAll('.news-card, .article-card, .news-grid article, .news-list article')].map((card) => {
      const a = card.querySelector('a'); const img = card.querySelector('img');
      return a ? { title: clean(card.innerText, 120), image: img?.src || '', url: a.href, source: 'Ciwara Infos' } : null;
    }).filter(Boolean);
    renderNewsStage(items);
  }

...
FEEDS=[('Malijet','https://malijet.com/rss'),('Maliweb','https://www.maliweb.net/rss/latest-posts'),('Africanews','https://fr.africanews.com/feed/rss?themes=news'),('Afrik.com','https://www.afrik.com/feed'),('Actualite.cd','https://actualite.cd/feed/')]
OUT=Path('data/news.json'); PER_SOURCE=8
...
OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps({'generatedAt':datetime.now(timezone.utc).isoformat(),'items':unique[:40],'sources':[x[0] for x in FEEDS],'errors':errors},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print(f'Radio Ciwara RSS: {len(unique[:40])} actualites; erreurs: {len(errors)}')
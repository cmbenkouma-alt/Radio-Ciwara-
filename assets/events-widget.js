(() => {
  ...
  // L'ancien bloc « À NE PAS MANQUER » devient le bloc vidéo Radio Ciwara.
  ...
  fetch('data/videos.json?v=' + Date.now())
    .then(r => r.json())
    .then(data => {
      const list = (data.items || []).filter(v => v.active !== false).slice(0,4);
      const el = videos.querySelector('.cv-list');
      if (!list.length) {
        el.innerHTML = '<div class="cv-empty">Les émissions vidéo de Radio Ciwara apparaîtront ici.<br><a href="ciwara-tv.html">OUVRIR CIWARA TV →</a></div>';
        return;
      }
      el.innerHTML = list.map(v => `
        <a class="cv-card" href="${v.link || 'ciwara-tv.html'}" target="${v.external ? '_blank' : '_self'}" rel="${v.external ? 'noopener' : ''}">
          <div class="cv-thumb">${v.image ? `<img src="${v.image}" alt="${v.title || 'Émission vidéo Ciwara'}" loading="lazy">` : '<span></span>'}<i class="cv-play">▶</i></div>
          <b>${v.title || 'Émission Ciwara'}</b>
          ${v.description ? `<small>${v.description}</small>` : ''}
        </a>`).join('') + '<a class="cv-more" href="ciwara-tv.html">VOIR TOUTES LES VIDÉOS →</a>';
    })

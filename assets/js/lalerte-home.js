(function(){
  ...
  function imagePath(p){
    var s=String(p||'');
    if(/^assets\/lalerte\/n180\/page-0(\d)\.jpg$/i.test(s)) s=s.replace(/page-0(\d)\.jpg$/i,'page-$1.jpg');
    return s;
  }
  function init(){
    if(document.querySelector('[data-lalerte-section]')) return;
    fetch('data/lalerte-180.json?v=20260915-4',{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('LALERTE data');return r.json()}).then(function(d){
      ...
      var cards=d.pages.slice(0,4).map(function(p){
        ...
        var img=esc(imagePath(p.image));
        return '<a class="lalerte-card" href="lalerte.html#page-'+p.page+'"><img loading="lazy" decoding="async" src="'+img+'" alt="'+title+' — L’ALERTE N°180"><div class="lalerte-card-body"><span>PAGE '+esc(p.page)+'</span><h3>'+title+'</h3><p>'+ex+'</p></div></a>';
      }).join('');
      ...
      var link=document.createElement('link');link.rel='stylesheet';link.href='assets/css/lalerte.css?v=20260915-4';document.head.appendChild(link);
